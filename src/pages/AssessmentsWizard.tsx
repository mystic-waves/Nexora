...
export default function AssessmentsWizard() {
  ...
  const canNext = useMemo(() => {
    switch (step) {
      ...
      case 2: return (answers.q3?.length ?? 0) > 0
      case 3: return !!answers.q4
      case 4: return !!answers.q5
      case 5: return !!answers.q6
      case 6: return !!answers.q7
      case 7: return !!answers.q8
      case 12: return (answers.q10?.trim().length ?? 0) > 0
      case 13: return (answers.q11?.trim().length ?? 0) > 0
      default: return true // sliders are optional
    }
  }, [step, answers])
        ...
        <div className="mt-6 max-w-2xl mx-auto">
          <div className="card">
            <div className="card-body space-y-6">
              <div className="text-xs text-slate-400">Step {step + 1} of {total}</div>
              {steps[step]}
              <div className="flex items-center justify-between">
                <button type="button" className={`btn-ghost ${step === 0 ? 'opacity-60 cursor-not-allowed' : ''}`} onClick={()=> step > 0 && setStep(step - 1)} disabled={step === 0}>← Prev</button>
                {!isLast ? (
                  <button type="button" className={`btn-primary ${!canNext ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!canNext} onClick={()=> setStep(step + 1)}>Next →</button>
                ) : (
                  <button type="button" className={`btn-primary ${!canNext ? 'opacity-60 cursor-not-allowed' : ''}`} disabled={!canNext} onClick={()=> setSubmitted(true)}>Analyze & Generate Career Map</button>
                )}
              </div>
            </div>
          </div>
...
function analyze(a: AnswerMap){
  ...
  return { personality, communication, learning, riasec, value, environment, recommendations: recs }
}
